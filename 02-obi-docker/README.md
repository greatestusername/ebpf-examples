# Phases 1 & 2: Zero-Code APM with Docker

> Run three "naked" microservices, see that APM is empty, then add one OBI container and watch full distributed traces appear -- without changing a single line of application code.
...
1. **Infrastructure Monitoring**: The `workshop.heartbeat` custom metric should be ingested. Search for it in [Metric Finder](https://app.signalfx.com/#/metrics). You should see `host.name` matching yours as an attribute. Search for that `host.name` in the Metric Finder to see what other metrics you are sending.
2. **APM**: Navigate to APM and filter by the environment you defined. It should be **empty (or not exist)**. No services, no traces, no service map.
...
> **Common mistake**: Make sure `obi:` is indented 2 spaces (same level as `frontend:`, `load-generator:`, etc.). If it's at the far left with no indentation, Docker Compose will reject it with `Additional property obi is not allowed`. It must be **inside** the `services:` block.
...
If you get stuck, the complete final `docker-compose.yaml` with all changes applied is in [`docker-compose.final.yaml`](docker-compose.final.yaml).

---

## Extending the Workshop

See [LLM.md](../LLM.md) for ideas on using an LLM to add new endpoints, services, error scenarios, and more to this project.
